...
console.log(process.env.MESSAGES_FILE);

fs.createReadStream(appDir.concat('/messagesFile'))
...
const errmessage =
  'Sorry. You have replied with an unrecognized text. Please use one of the below buttons to interact with this bot';
...
bot.on('message', (msg) => {
  ...
  if (messageMap.get(inputtext) != undefined) {
    ...
    logger.info(`${msg.chat.id}#${msg.from.first_name}#${inputtext}`);
    ...
    msg.text.toString().startsWith('/start') ||
    msg.text.toString().toLowerCase() == 'hi' ||
    msg.text.toString().toLowerCase() == 'hello'
  ) {
    logger.info(
      `${msg.chat.id}#${msg.from.username}#${msg.from.first_name}#Start`
    );
    message =
      'Welcome ' +
      msg.from.first_name +
      '. Please use one of the options presented below the chat window' +
      '\n\n' +
      'Note: The information is shared in good intent and to help residents. Residents need to leverage their own discretion. Information may change time to time. Management Committee is neither responsible nor monitoring the external web links.';
  } else {
    logger.info(
      `${msg.chat.id}#${msg.from.username}#${
        msg.from.first_name
      }#Error input#${msg.text.toString()}`
    );
    message = errmessage;
  }
